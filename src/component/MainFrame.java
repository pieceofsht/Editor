...
public class MainFrame {
	/**
	 * JFrame frmToiletEditor: 主应用程序的主框架
	 * public JTextArea jta: 用户可以输入和编辑文本的文本区域
	 * private JPanel panel: 包含其他组件的主面板
	 * private NumberPanel np: 显示行号的面板
	 * private MyMenuItem menuItem_1, menuItem_2, menuItem_3, mntmNewMenuItem, menuItem_5,
	 *   menuItem_6, radioButtonMenuItem, menuItem_7, menuItem_8,
	 *   menuItem_9, cut_, copy_, paste_, delete_, select_all_: 用于各种功能的菜单项，如剪切、复制、粘贴、删除等
	 * private JMenu mnNewMenu: 应用程序的主菜单
	 * JLabel lblNewLabel_2, lblNewLabel_3, lblNewLabel_4: 应用程序中使用的标签
	 * boolean edited = false: 检查文本区域内容是否已被编辑的标志
	 * private UndoManager um: 处理撤销和重做操作的管理器
	 * static int lines = 1: 跟踪文本区域中的行数的变量
	 * String filepath: 当前打开文件的路径
	 * static MainFrame window: 主框架的实例
	 * MyPopupMenu pop = new MyPopupMenu(): 右键单击时出现的弹出菜单
	 */
	JFrame frmToiletEditor;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() { //创建一个线程，将窗口放入其中
			public void run() {
				try {
					window = new MainFrame();
					window.frmToiletEditor.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		frmToiletEditor = new JFrame();
		frmToiletEditor.setTitle("ToiletEditor");
		frmToiletEditor.setFont(new Font("微软雅黑", Font.PLAIN, 12));
		frmToiletEditor.setBounds(100, 100, 635, 500);
		frmToiletEditor.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
		frmToiletEditor.getContentPane().setLayout(null);
		frmToiletEditor.setResizable(false);
		frmToiletEditor.setLocationRelativeTo(null);
		frmToiletEditor.setIconImage(new ImageIcon(MainFrame.class.getResource("/img/notepad.png")).getImage());
		frmToiletEditor.addWindowListener(new WindowListener() {
			...
			public void windowClosing(WindowEvent e) {
				if (!edited)
					...
					new MyDialog3(frmToiletEditor, "Save");
				}
			}
		...
		frmToiletEditor.getContentPane().add(menuBar);

		mnNewMenu = new MyMenu("\u6587\u4EF6(F)"); //此处为Bar上的文件按钮，之后是
		mnNewMenu.setMnemonic('F');
		menuBar.add(mnNewMenu);

		JMenuItem menuItem = new MyMenuItem("\u65B0\u5EFA"); //新建文件
		...
		menuItem_1 = new MyMenuItem("\u6253\u5F00"); //打开文件
		...
		menuItem_2 = new MyMenuItem("\u4FDD\u5B58"); //保存文件
		...
		menuItem_3 = new MyMenuItem("\u53E6\u5B58\u4E3A"); //另存为文件
		...
		JMenuItem menuItem_4 = new MyMenuItem("\u9000\u51FA"); //退出
		menuItem_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!edited)
					System.exit(0);
				else {
					new MyDialog3(frmToiletEditor, "Save"); //弹窗询问是否保存改动
				}
			}
		});
		...
		JMenuItem mntToiletEditor = new MyMenuItem("\u5173\u4E8EToiletEditor");
		mntToiletEditor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				new AuthorDialog(frmToiletEditor, "厕所清洁工");
			}
		});
		mntToiletEditor.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F1, 0));
		mns.add(mntToiletEditor);
		...
		menuItem_10.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				SettingMenu stm = new SettingMenu(frmToiletEditor,"个性设置",jta,np);
			}
		});
		mnt.add(menuItem_10);
					...
					if (!result) {
						...
						if (KeyEvent.getKeyText(e.getKeyCode()).equals("Enter")) {
							...
							try {
								...
								int lineNumber = jta.getLineOfOffset(caretPosition);
								int startOffset = jta.getLineStartOffset(lineNumber);
								int endOffset = jta.getLineEndOffset(lineNumber);
								String lineText = jta.getDocument().getText(startOffset, endOffset - startOffset).trim();

								if (lineText.isEmpty() && lineNumber != 0) {
									lines = (jta.getLineCount() - 1 >= 1) ? jta.getLineCount() - 1 : 1;
								}
		...
		scrollPane.setBounds(20, 59, 599, 403);
		...
		frmToiletEditor.getContentPane().add(scrollPane);
		...
		frmToiletEditor.getContentPane().add(panel);
		...
		lblNewLabel.setIcon(new ImageIcon(MainFrame.class.getResource("/img/new.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel);
		...
		lblNewLabel_1.setIcon(new ImageIcon(MainFrame.class.getResource("/img/open.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel_1);
		...
		lblNewLabel_2.setIcon(new ImageIcon(MainFrame.class.getResource("/img/save.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel_2);
		...
		lblNewLabel_3.setIcon(new ImageIcon(MainFrame.class.getResource("/img/undo.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel_3);
		...
		lblNewLabel_4.setIcon(new ImageIcon(MainFrame.class.getResource("/img/redo.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel_4);
		...
		lblNewLabel_5.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if(e.getButton()==MouseEvent.BUTTON1)
					new SettingMenu(frmToiletEditor,"个性设置",jta,np);
			}
		...
		lblNewLabel_5.setIcon(new ImageIcon(MainFrame.class.getResource("/img/settings.png")));
		frmToiletEditor.getContentPane().add(lblNewLabel_5);
	}

	public void newFile() {
		if (edited) {
			new MyDialog(frmToiletEditor, "Save");
		} else {
			lines = 1;
			np.hoff = 1;
			jta.setText("");
	...
	public void open() {
		if (!edited) {
			FileDialog dia1 = new FileDialog(frmToiletEditor, "打开"); // 重要
			...
			new MyDialog2(frmToiletEditor, "Save");
		}
	}
	...
	public void saveToAnotherPath() {
		FileDialog dia2 = new FileDialog(frmToiletEditor, "保存", FileDialog.SAVE);
